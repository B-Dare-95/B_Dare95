...
class LayerPickerWindow(Window):
    ...
    def __init__(self, wall, layer_data_list):
        ...
        self._auto_names = {}
        self.apply_to_all = [False]  # mutable container; [True] = all walls of type
    ...
    def _render_phase3(self):
        ...
        # ── Scope toggle ─────────────────────────────────────────────────
        toggle_card = Border()
        toggle_card.Background      = brush(C_SURFACE)
        toggle_card.CornerRadius    = System.Windows.CornerRadius(7)
        toggle_card.Padding         = Thickness(10, 8, 10, 8)
        toggle_card.Margin          = Thickness(0, 0, 0, 10)
        toggle_card.BorderThickness = Thickness(1)
        toggle_card.BorderBrush     = brush(C_MUTED)

        toggle_inner = StackPanel()
        toggle_inner.Orientation = Orientation.Vertical

        scope_lbl        = make_text("Apply to", 11, C_SUBTEXT, bold=True)
        scope_lbl.Margin = Thickness(0, 0, 0, 6)
        toggle_inner.Children.Add(scope_lbl)

        opts_row             = StackPanel()
        opts_row.Orientation = Orientation.Horizontal

        def _make_scope_opt(label, is_active):
            b = Border()
            b.Padding         = Thickness(14, 5, 14, 5)
            b.CornerRadius    = System.Windows.CornerRadius(5)
            b.Cursor          = System.Windows.Input.Cursors.Hand
            b.Background      = brush(C_ACCENT  if is_active else C_CARD)
            b.BorderBrush     = brush(C_ACCENT  if is_active else C_MUTED)
            b.BorderThickness = Thickness(1)
            b.Margin          = Thickness(0, 0, 6, 0)
            b.Child           = make_text(label, 12,
                                          C_BG      if is_active else C_SUBTEXT,
                                          bold=is_active)
            return b

        is_all = self.apply_to_all[0]
        btn_single = _make_scope_opt("Selected wall only",    not is_all)
        btn_all    = _make_scope_opt("All walls of this type",    is_all)

        def on_scope_single(s, e):
            self.apply_to_all[0] = False
            self._refresh()

        def on_scope_all(s, e):
            self.apply_to_all[0] = True
            self._refresh()

        btn_single.MouseLeftButtonUp += on_scope_single
        btn_all.MouseLeftButtonUp    += on_scope_all

        opts_row.Children.Add(btn_single)
        opts_row.Children.Add(btn_all)
        toggle_inner.Children.Add(opts_row)
        toggle_card.Child = toggle_inner
        self.layer_stack.Children.Add(toggle_card)
        # ── end scope toggle ─────────────────────────────────────────────
    ...
    def get_result(self):
        ...
        return groups, self.host_key, name_map, self.apply_to_all[0]
...
groups, host_key, name_map, apply_to_all = win.get_result()
...
if apply_to_all:
    all_walls    = FilteredElementCollector(doc)\
                       ...
                       .ToElements()
    target_walls = [w for w in all_walls
                    if get_id_value(w.WallType.Id) == get_id_value(base_type.Id)]
else:
